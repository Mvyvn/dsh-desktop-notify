...
Verify: switch to another window, let the agent run a small task, and a toast should appear in the bottom-right corner; while the DSH page is focused, nothing pops (a focused page left untouched for 2 minutes counts as unfocused and resumes pushing). You can also run the smoke test directly:
...
```
dsh-desktop-notify/
├── lib/          # host index.js (gating / queue) + winrt.js (Windows sender) + browser client.js (focus reporting)
