...
const PY_SRC = [
  ...
  "    icon = Icon(path=Path(p['i'])) if p.get('i') else None",
...
export function apply(ctx) {
  ...
  function locPrefix(extraTitle) {
    const ws = state.workspaceName
    if (ws) return extraTitle ? ws + '/' + extraTitle : ws
    return extraTitle || ''
  }
  // 子会话 → 主会话标题（经 header.parentSession 回溯；取不到退化为子会话标题）
  function mainSessionTitleFor(subId) {
    try {
      const sessions = ctx.get('sessions')
      if (sessions === undefined || typeof sessions.get !== 'function') return sessionTitleText(subId)
      const sub = sessions.get(subId)
      if (!sub) return sessionTitleText(subId)
      const parent = sub.header && sub.header.parentSession
      if (!parent) return sessionTitleText(sub)
      const main = typeof parent === 'string' ? sessions.get(parent) : parent
      return main ? sessionTitleText(main) : sessionTitleText(sub)
    } catch (e) {
      return sessionTitleText(subId)
    }
  }
    ...
    try {
      ...
      if (type === 'assistant/message') {
        ...
        if (info && data.outcome === 'rejected') {
          ...
          notify('🚫 操作被自动拒绝', (locPrefix(t) ? locPrefix(t) + ':' : '') + (info.tool || '工具') + '-' + (info.reason || '操作被自动拒绝'), 'normal')
        }
      }
    } catch (e) {
  ...
  ctx.on('agent/status', (payload) => {
    try {
      ...
      const cancel = later(() => {
        ...
        // 消息格式：工作区/会话名:结尾输出内容（用户可读、可定位到哪个工作区哪个会话）
        notify('✅ DSH 任务完成', (locPrefix(title) ? locPrefix(title) + ':' : '') + summary, 'low')
      }, 3000)
  ...
  ctx.on('tools/execute', (exec, next) => {
    try {
      if (exec && exec.name === 'ask_user_question') {
        ...
        notify('❓ DSH 等待你的输入', (locPrefix(t) ? locPrefix(t) + ':' : '') + (h ? '[' + h + '] ' : '') + q, 'normal')
      }
    } catch (e) {
  ...
  ctx.on('subagent/end', (info) => {
    try {
      ...
      // 前缀 = 工作区/主会话名，正文 = 子代理名已完成
      notify('🤖 后台子代理结束', (locPrefix(mainSessionTitleFor(subId)) ? locPrefix(mainSessionTitleFor(subId)) + ':' : '') + (sessionTitleText(subId) || subId || '后台子代理') + '已完成', 'low')
    } catch (e) { /* ignore */ }
  })
  ...
  ctx.on('goal/changed', (payload) => {
    try {
      ...
      if (change.operation === 'complete') {
        notify('🎯 目标已完成', (locPrefix(t) ? locPrefix(t) + ':' : '') + objective + '-已完成', 'normal')
      } else if (change.operation === 'block') {
        const br = goal.blockedReason
        notify('🎯 目标已阻塞', (locPrefix(t) ? locPrefix(t) + ':' : '') + objective + (br && br.message ? '-' + String(br.message).slice(0, 160) : ''), 'normal')
      }
    } catch (e) { /* ignore */ }
  })
  ...
  if (jobs !== undefined && typeof jobs.onJobDone === 'function') {
    ctx.effect(() => jobs.onJobDone((snapshot, owner) => {
      try {
        ...
        // 前缀 = 工作区/主会话名（owner 的会话优先，其次 ownerSession id）
        const mainTitle = sessionTitleText(owner && owner.session) || sessionTitleText(s.ownerSession) || ''
        notify(
          '🧰 后台任务结束',
          (locPrefix(mainTitle) ? locPrefix(mainTitle) + ':' : '') + String(s.label || s.id || '后台任务') + '已完成',
          'low',
        )
      } catch (e) { /* ignore */ }
    }))
