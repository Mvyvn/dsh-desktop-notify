...
/** 每次进程随机生成的点击令牌：/dnotify/click 靠它防止任意网页盲触发跳转。 */
function randomToken() {  try {
...
export function apply(ctx, config) {
  ...
  //   GET  /dnotify/events      text/event-stream：把待处理跳转推给已打开的页面
  //   POST /dnotify/claim       { pageId } → 谁先认领谁执行跳转（保证只切一个页面）
  //   GET  /dnotify/click?t=<进程令牌>&target=<目标> → 通知点击落地页：记录目标 + 自关
  // 除 /click 外都过 connection.admit()（Host/Origin 栅栏 + 浏览器鉴权）；/click 用
  // 每次进程随机生成的令牌校验（防止任意网页靠 <img> 之类盲触发）。
  const CLICK_TOKEN = randomToken()
  const PENDING_TTL_MS = 30000
  /** 待认领的跳转（同一时刻只留最新一个）+ SSE 订阅者。 */
  let pendingOpen = null
  const openStreams = new Map()
  function publishOpen(target) {
    const envelope = { target, at: Date.now() }
    pendingOpen = envelope
    const payload = `event: navigate\ndata: ${JSON.stringify(envelope)}\n\n`
    for (const [res, stopPing] of [...openStreams]) {
      try {
        res.write(payload)
      } catch (e) {
  ...
  function expiredOpen() {
    if (pendingOpen && Date.now() - pendingOpen.at > PENDING_TTL_MS) pendingOpen = null
    return pendingOpen
  }
  ...
  function clickLandingPage(target) {
    ...
    return '<!doctype html><meta charset="utf-8"><title>DSH 通知</title>'
      ...
      + '<p>已通知 DSH 切换</p><script>try{window.close()}catch(e){}</script>'
      + `<!-- target=${safe} -->`
  }
  async function serveNotifyRoute(req, res) {
    ...
    // 点击落地：由系统/浏览器直接打开，没有 Origin，用进程令牌校验。
    ...
    if (path === 'click' && method === 'GET') {
      ...
      if (isValidTarget(target)) {
        publishOpen(target)
        log(`[dsh-desktop-notify] click -> ${target} pages=${openStreams.size}`)
      } else if (target) {
        log(`[dsh-desktop-notify] 点击目标非法，已忽略: ${target}`)
      }
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' })
      res.end(clickLandingPage(target))
      return
    }
    ...
    if (path === 'events' && method === 'GET') {
      ...
      const current = expiredOpen()
      if (current) res.write(`event: navigate\ndata: ${JSON.stringify(current)}\n\n`)
    ...
    if (path === 'claim' && method === 'POST') {
      ...
      const current = expiredOpen()
      if (!current) { json(res, 200, { ok: false, reason: 'none' }); return }
      // 先到先得：认领成功即清空，保证多页面时只有一个页面执行跳转
      pendingOpen = null
      log(`[dsh-desktop-notify] claim by ${(body && body.pageId) || '?'} -> ${current.target}`)
      json(res, 200, { ok: true, target: current.target })
      return
    }
    ...
    if (path === 'page-focus' && method === 'POST') {
      ...
      if (body.focused) gate.setPage(pageId, Date.now(), sessionId)
