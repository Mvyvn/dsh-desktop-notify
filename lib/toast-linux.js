...
function flushPending(s) {
  ...
  for (const frame of items) {
    try { s.socket.write(frame) } catch (e) { /* ignore */ }
  }
}
...
function createSession() {
  ...
  socket.on('error', (err) => {
    ...
    if (s.lastError !== reason) {
      s.lastError = reason
      console.error(`[dsh-desktop-notify] D-Bus 连接错误（${path}）:`, reason)
    }
  })
  socket.on('close', () => {
    s.ready = false
...
export function sendToast(item) {
  ...
  if (!session || session.failed) session = createSession()
  const s = session
  if (s.ready) {
    try {
      s.socket.write(frame)
    } catch (e) {
      console.error('[dsh-desktop-notify] D-Bus 写入失败:', e && e.message)
      session = null
    }
    return
  }
  if (s.pending.length >= MAX_PENDING) s.pending.shift()
