...
import { buildToastXml } from './toast-xml.js'
import {
...
// 1.6.1 这里曾实现过 `dsh-notify:` 自定义协议 + 隐藏 PowerShell 一跳，用来让点击
// "不新开标签页"。用户判定这种做法低效（每次点击起一个进程）并要求移除，现已删除：
// 通知的点击链接统一走 http 落地页（见 lib/index.js 的 deepLink / clickLandingPage），
// 接受浏览器新开一个标签页。本文件不再写注册表里的协议键。

/**
 * 发送一条 Windows Toast（同步；失败抛 Error）。
 * @param {{title: string, message: string, iconPath?: string, url?: string}} item
 *   url 非空时用**协议激活**（`activationType="protocol"`）：点击由系统直接打开该地址，
 *   不需要注册 COM 激活器（`INotificationActivationCallback`）；浏览器若已有同地址的
 *   DSH 页面会切到那一个。url 为空则是普通 Toast——点击只会消失，不做任何跳转。
 */
export function sendToast(item) {
  ...
  const xml = buildToastXml({
    ...
    message: truncateText(item.message || '', 400),
    iconUri: icon ? toFileUri(icon) : '',
